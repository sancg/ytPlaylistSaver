...
async function getPlaylistTab(): Promise<GetPlaylistCall> {
  ...
  if (tab.id) {
    const { playlist, skipVideos, error } = await sendMessageTab<GetPlaylistCall>(tab.id, {
      action: cs.EXTRACT_PLAYLIST,
    });

    return { playlist, skipVideos, error };
  }

  return { playlist: [], skipVideos: [], error: 'No response was received' };
}
